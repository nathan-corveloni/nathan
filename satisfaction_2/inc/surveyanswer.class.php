...
class PluginSatisfactionSurveyAnswer extends CommonDBChild {
   ...
   static function displayAnswer($question, $value = 0) {
      ...
      switch ($question['type']) {
         ...
         case PluginSatisfactionSurveyQuestion::EMOJIESCALE:
            self::showEmojiAnswer($question, $value);
            break;
      }
   }

   /**
    * Star display (agora com emojis)
    *
    * @param     $question
    * @param int $value
    */
   static function showStarAnswer($question, $value = 0) {
      ...
      // Lista de emojis e seus significados
      $emojis = [
         '1' => ['emoji' => '😞', 'label' => __('Muito Ruim', 'satisfaction')],
         '2' => ['emoji' => '☹️', 'label' => __('Ruim', 'satisfaction')],
         '3' => ['emoji' => '😐', 'label' => __('Regular', 'satisfaction')],
         '4' => ['emoji' => '🙂', 'label' => __('Boa', 'satisfaction')],
         '5' => ['emoji' => '😊', 'label' => __('Muito Boa', 'satisfaction')],
      ];

      echo "<div class='emoji-rating' style='display: flex; gap: 15px; align-items: center;'>";
      foreach ($emojis as $key => $data) {
         $checked = ($key == $value) ? "checked" : "";
         echo "<label style='text-align: center;'>";
         echo "<input type='radio' name='answer[$questions_id]' value='$key' $checked>";
         echo "<div style='font-size: 24px; margin: 5px;'>{$data['emoji']}</div>";
         echo "<small style='display: block;'>{$data['label']}</small>";
         echo "</label>";
      }
      echo "</div>";
...
echo "<style>
    ...
    .emoji-rating input[type='radio']:checked + div { 
    ...
    .emoji-rating div { 
        transition: transform 0.2s, opacity 0.2s, filter 0.2s !important; 
        opacity: 1 !important; 
        filter: none !important; 
    }
    /* Escurecer os emojis não selecionados */
    .emoji-rating input[type='radio']:not(:checked) + div { 
   ...
   /**
    * Exibe a resposta do tipo Emoji Scale
    *
    * @param array $question Dados da questão
    * @param int $value Valor da resposta
    */
   static function showEmojiAnswer($question, $value = 0) {
      $questions_id = $question['id'];
      ...
      foreach ($emojis as $key => $data) {
         ...
         echo "<label style='text-align: center;'>";
         echo "<input type='radio' name='answer[$questions_id]' value='$key' $checked>";
         echo "<div style='font-size: 24px; margin: 5px; display: inline-block;'>{$data['emoji']}</div>";
         echo "<small style='display: block;'>{$data['label']}</small>";
         echo "</label>";
      }
      echo "</div>";
...
echo "<style>
   ...
   static function getAnswer($question, $value = 0) {
      ...
      switch ($question['type']) {
         ...
         case PluginSatisfactionSurveyQuestion::NOTE :
            $emojis = [
               '1' => '😞 ' . __('Muito Ruim', 'satisfaction'),
               '2' => '☹️ ' . __('Ruim', 'satisfaction'),
               '3' => '😐 ' . __('Regular', 'satisfaction'),
               '4' => '🙂 ' . __('Boa', 'satisfaction'),
               '5' => '😊 ' . __('Muito Boa', 'satisfaction'),
            ];
            return isset($emojis[$value]) ? $emojis[$value] : $value;
         ...
         case PluginSatisfactionSurveyQuestion::EMOJIESCALE:
            ...
            return isset($emojis[$value]) ? $emojis[$value] : __('Unknown', 'satisfaction');
      }
   }
