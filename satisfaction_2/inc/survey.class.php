...
class PluginSatisfactionSurvey extends CommonDBTM {
    ...
    *
    * @param array $responses Respostas enviadas pelo usuário
    * @return bool
    */
   function processResponses($responses) {
      ...
      foreach ($responses as $question_id => $response) {
         ...
         $question = new PluginSatisfactionSurveyQuestion();
         if (!$question->getFromDB($question_id)) {
            continue; // Ignorar se a questão não for encontrada
         }

         switch ($question->fields['type']) {
            case PluginSatisfactionSurveyQuestion::YESNO:
               // Salvar resposta para questões do tipo Yes/No
               $this->saveResponse($question_id, $response);
               break;

            case PluginSatisfactionSurveyQuestion::TEXTAREA:
               // Salvar resposta para questões do tipo Textarea
               $this->saveResponse($question_id, $response);
               break;

            case PluginSatisfactionSurveyQuestion::NOTE:
               // Salvar resposta para questões do tipo Nota (agora com emojis)
            ...
            case PluginSatisfactionSurveyQuestion::EMOJIESCALE:
               // Salvar resposta para questões do tipo Emoji Scale
               if (in_array($response, ['1', '2', '3', '4', '5'])) {
                  $this->saveResponse($question_id, $response);
               }
               break;
    ...
    *
    * @param int $question_id ID da questão
    * @param mixed $response Resposta fornecida pelo usuário
    * @return bool
    */
   private function saveResponse($question_id, $response) {
      global $DB;

      $query = "INSERT INTO glpi_plugin_satisfaction_surveyanswers
                (plugin_satisfaction_surveyquestions_id, response, date_creation)
                VALUES (:question_id, :response, NOW())";
